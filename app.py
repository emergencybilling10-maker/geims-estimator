...
def load_geims_data():
    try:
        # This looks for your uploaded Excel file
        df = pd.read_excel("Geims Hospital...xlsx")
        # Clean column names
...
if df_surgery is not None:
    with st.sidebar:
        ...
        st.divider()
        st.info("Policy: Package inclusive of Room, Diet, Nursing, and RMO for Package Days.")

    # Dynamic Search across ALL Excel data
    col1, col2 = st.columns(2)
    with col1:
        # Adjust 'Department' to match your Excel column name exactly
        sel_dept = st.selectbox("Search Department", sorted(df_surgery['Department'].dropna().unique()))
    with col2:
        # Adjust 'Service Name' to match your Excel column name exactly
    ...
    # Ensure your Excel has a column for 'Package Days'
    ...
    st.warning(f"Please upload 'Geims Hospital...xlsx' to GitHub and refresh.")
