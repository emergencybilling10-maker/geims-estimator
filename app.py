...
# Daily rates for extra days only as per GEIMS 2025 Policy
ROOM_POLICY = {
...
st.subheader("Automated Tariff Sync | Graphic Era Institute of Medical Sciences")

# Syncing with your uploaded PDF
...
if df_master is not None:
    with st.sidebar:
        ...
        st.divider()
        st.info("System recognizes Inclusive Packages: covers Room, Diet, and Nursing for Pkg Days.")

    try:
        # Auto-detect Department and Service columns
        ...
        # Package Days detection (defaults to 1 if not specified in PDF)
        ...
        # Breakdown table following GEIMS 2025 Manual
        breakdown = {
            f"Package Rate ({sel_proc})": base_rate,
            "Package Inclusions": f"Includes Room, Nursing, RMO & Diet for {pkg_days} days",
            "Admission / MRD Fee": 450.0 # Fixed charge as per policy
        }
        ...
        st.warning(f"Formatting Check: Please ensure your PDF has 'Department' and 'Service Name' columns. Detail: {e}")
else:
    st.error("System could not find 'Geims_Tariff_Dec_2025.pdf' in the repository.")
